...
def fit(X, y, w=None, **kwargs):
    ...
    while algo.nbasis() < max_terms and epoch < max_epochs:
        ...
        m0 = m = algo.nbasis()
        if len(results) > 0:
            results = np.array(results, dtype=[('i','i4'),('j','i4'),('t','f8'),('dm','i4'),('mse','f8')])
            ...
            for htype in (['l'] if dm==1 else ['+','-']):
                ...
                if mse > 0:
                    ...
                    basis_sse.append(np.inf)
                    m = m+1
            assert algo.nbasis() == len(basis) == m

        _dump_logger_row(logger, epoch, len(basis), dt, model[algo.nbasis()-1], labels)
